...
def is_admin():
    ...
    try:
        with winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE", 0, winreg.KEY_WRITE
        ):
            return True
...
def check_sapi4_installation():
    try:
        ...
        # Check if CLSID is registered
        clsid_key_path = (
            r"SOFTWARE\Classes\CLSID\{A910187F-0C7A-45AC-92CC-59EDAFB77B53}"
        )
        with winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, clsid_key_path, 0, winreg.KEY_READ
        ):
...
class TOKEN_PRIVILEGES(ctypes.Structure):
    _fields_ = [("PrivilegeCount", ctypes.c_ulong), ("Privileges", LUID * 1)]
...
def take_ownership_and_set_permissions(key_path: str):
    ...
    try:
        ...
        print(
            f"Could not modify permissions for {key_path}. Ensure you have administrative privileges."
        )
    except Exception as e:
