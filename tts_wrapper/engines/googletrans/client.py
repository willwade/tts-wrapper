...
class GoogleTransClient:
    ...
    def _create_wav_header(self, output: BytesIO, sample_rate: int, nchannels: int) -> bytes:
        """
        Creates a WAV header based on the MP3 properties (sample rate, channels, etc.).
        """
        # Set WAV format constants
        num_samples = output.tell() // (nchannels * 2)  # Calculate number of samples based on current size
        wav_header = BytesIO()

        # Write WAV header
        wav_header.write(b'RIFF')
        wav_header.write((36 + num_samples * nchannels * 2).to_bytes(4, 'little'))  # Chunk size
        ...
        wav_header.write((num_samples * nchannels * 2).to_bytes(4, 'little'))  # Subchunk 2 size
