...
import json
import struct
from ctypes import POINTER, c_bool, c_char_p, c_int, c_uint8
...
def log_callback(log_message_ptr):
    if log_message_ptr:
        log_message = ctypes.cast(log_message_ptr, ctypes.c_char_p).value.decode(
            "utf-8"
        )
...
def word_timing_callback(word_timings_ptr):
    if word_timings_ptr:
        word_timings_json = ctypes.cast(word_timings_ptr, c_char_p).value.decode(
            "utf-8"
        )
...
class Synthesizer:
    ...
    def synthesize_to_bytes(
        ...
        if streaming and not byte_callback:
            msg = "streaming=True requires a byte_callback."
        ...
        if result_ptr:
            ...
            # Get audio data
            audio_data_ptr = ctypes.addressof(result_ptr.contents) + ctypes.sizeof(
                c_int
            )
...
if __name__ == "__main__":
    ...
    for voice in TTSManager.get_voices():
        print(
            f"ID: {voice['id']}, Name: {voice['name']}, Language: {voice['language']}"
        )
    ...
    if audio_data:
        print(
            f"Audio data received, length: {len(audio_data)}, sample rate: {sample_rate}"
        )
