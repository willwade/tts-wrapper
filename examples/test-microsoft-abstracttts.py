...
def main():
    ...
    # Verify that the TTS engine inherits from AbstractTTS
    print(f"MicrosoftTTS is an instance of AbstractTTS: {isinstance(tts, AbstractTTS)}")
    ...
    # ISO 639-3 format
    voices_iso = tts.get_voices(langcodes="iso639_3")
    if voices_iso:
        print(f"ISO 639-3 format: {voices_iso[0]['language_codes']}")

    # Human-readable display names
    voices_display = tts.get_voices(langcodes="display")
    if voices_display:
        print(f"Display names: {voices_display[0]['language_codes']}")

    # All formats
    voices_all = tts.get_voices(langcodes="all")
    if voices_all:
    ...
    try:
        ...
        audio_bytes = tts.synth_to_bytes("This is a test of the Microsoft TTS engine.")
    ...
    try:
        ...
        tts.synth("This is a test of the Microsoft TTS engine.", temp_path)
